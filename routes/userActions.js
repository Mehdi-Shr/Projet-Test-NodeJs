...
module.exports = {
  async register(req, res) {
    ...
    return res.status(400).json({
      message:
        "Pour s'inscrire il faut renseigner : email, password, firstname, lastname",
    });
  },
  async login(req, res) {
    ...
    if (email && password) {
      try {
        ...
        return res
          ...
          .json({
            message:
              "Nous ne parvenons pas à réaliser la requette au niveau de la base de données",
          });
      }
    }
    return res
      .status(400)
      .json({
        message: "Pour se connecter, il faut renseigner : email, password",
      });
  },
  async get(req, res) {
    ...
    if (id) {
      ...
      if (firstname || lastname) {
        try {
          ...
          console.log(e);
          return res.json({
            message: "Nous n'avons pas réussi à modifier vos infromations",
          });
        }
      }
      return res
        .status(400)
        .json({ message: "il manque le firstname ou le lastname" });
    }
    return res.status(400).json({ message: "Veuillez vous authentifier" });
  },
  async updatePassword(req, res) {
    ...
    if (id) {
          ...
          if (user.password == oldPassword) {
            try {
              ...
              return res
                ...
                .json({
                  message:
                    "U  problème est survenu, veillez reéssayer ulterieurement",
                });
            }
          }
          return res
            .status(400)
            .json({ message: "Le mot de passe actuel est faux" });
        } catch (e) {
          console.log(e);
          return res
            .status(400)
      ...
      return res
        .status(400)
        .json({ message: "Veuillez renseigner le password et le newPassword" });
    }
    return res.status(400).json({ message: "Veuillez vous authentifier" });
  },
};
